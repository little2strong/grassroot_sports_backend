...
class LiveScoringController extends Controller
{
    ...
    public function recordBall(Request $request, int $matchId): JsonResponse
    {
        ...
        $validated['is_boundary_four'] = ($validated['event_type'] === 'run' && isset($validated['runs_scored']) && $validated['runs_scored'] == 4);
        $validated['is_boundary_six'] = ($validated['event_type'] === 'run' && isset($validated['runs_scored']) && $validated['runs_scored'] == 6);
        $validated['is_wicket_ball'] = ($validated['event_type'] === 'wicket');
        $validated['is_legal_delivery'] = !in_array($validated['event_type'], ['wide', 'no_ball']);
        
        if (in_array($validated['event_type'], ['wide', 'no_ball', 'bye', 'leg_bye', 'penalty'])) {
            $validated['extras_type'] = $validated['event_type'];
        } else {
            $validated['extras_type'] = null;
        }
    ...
    private function userCanScoreFixture(User $user, Fixture $fixture): bool
    {
        ...
        if ($fixture->club_id && $user->isClubOwner($fixture->club)) {
            return true;
        }
